...
export default function VerifyResult({ status, serial, product, socialLinks }: Props) {
  ...
  return (
    <>
      {/* iOS Safari paints the status-bar / home-indicator safe areas and the
          overscroll (rubber-band) region using the ROOT element's background,
          not a child layer - so the gradient has to live on <html> itself to
          reach every physical edge. Scoped to this route: the tag is removed
            ...
            <div className="relative mb-8 flex items-center justify-center">
              ...
              >
                {/* Plain img: Accurate's image host can change (see docs), so it
                    ...
                    mix-blend-multiply makes the photo's baked-in white margin
                    blend into the gradient (white * bg = bg) while the dark
                    product stays visible - no visible rectangle. */}
                <img
                  src={product.imageUrl}
                  alt={product.name}
                  width={480}
                  height={480}
                  className="relative object-contain max-w-[320px] max-h-[320px] w-full mix-blend-multiply"
                />
              </motion.div>
            </div>
          )}
